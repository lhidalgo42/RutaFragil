...
if ($Mode -eq 'tests') {
    foreach ($number in 1..3) {
        $result = Invoke-Recorded ('02_r6_run_' + $number) 'powershell.exe' @(
            '-NoProfile','-ExecutionPolicy','Bypass','-File','tools/run_tests.ps1') 700
        $results += $result
        [IO.File]::WriteAllText((Join-Path $PSScriptRoot '02_r6_runs.json'), ($results | ConvertTo-Json -Depth 5), $utf8)
    ...
    try {
        ...
        $result = Invoke-Recorded '02_r6_unsafe' 'powershell.exe' @(
            '-NoProfile','-ExecutionPolicy','Bypass','-File','tools/run_tests.ps1') 700
        [IO.File]::WriteAllText((Join-Path $PSScriptRoot '02_r6_unsafe_result.json'), ($result | ConvertTo-Json), $utf8)
    ...
    foreach ($run in @(
        @{name='06_r6_demo'; args=@('--headless','--fixed-fps','60','--path','.','-s','res://src/tooling/run_demo.gd'); timeout=120},
        @{name='06_r6_handling'; args=@('--headless','--path','.','-s','res://src/tooling/probe_handling.gd'); timeout=120},
        @{name='06_r6_net_long'; args=@('--headless','--path','.','-s','res://src/tooling/run_net_scenario.gd','++','role=launcher','waypoints=16'); timeout=350}
    )) {
        $result = Invoke-Recorded $run.name $enginePath $run.args $run.timeout
        $results += $result
        [IO.File]::WriteAllText((Join-Path $PSScriptRoot '06_r6_results.json'), ($results | ConvertTo-Json -Depth 5), $utf8)
        Write-Output ($result.name + '_EXIT=' + $result.exit)
        if ($result.exit -ne 0) { exit $result.exit }
    }
}
