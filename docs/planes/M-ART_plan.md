...
**Paso 3 — Validador del checklist (DA6).** Script que toma un `.glb` y responde sí/no a los puntos 1–5, con el número concreto que falla. Rojo primero: un glTF deliberadamente malo (escala ×100, pivote centrado, 5k tris) tiene que suspender en los cinco. **HECHO 2026-09-21:** `tools/glb_check.py` (Python puro, sin dependencias) y `tools/glb_check_test.py` (fixtures por Blender headless; rojo 7/7, verde 8/8). Aplica las transformaciones de nodo del glTF antes de medir. La señal de «normales invertidas» son tres: aristas dirigidas recorridas dos veces, volumen con signo negativo, y normal almacenada contra el winding — la tercera es la que caza los generadores, que hornean normales aparte de la topología (386 caras en el furgón de prueba). El furgón de prueba falla 5/8: escala 0,99 m, pivote centrado, 2 caras sueltas, normales, texturas 2048². Es el caso rojo real.

**Paso 4 — Un asset de punta a punta (DA5).** El paquete, desde la generación hasta verlo en el Playground, con colisión autorada, pasando el validador y registrado en `CREDITS.md`. Si DA4 no está resuelta, el paso 4 se hace con el mesh de exploración **fuera del repo** para probar el camino, y se repite con el 3D limpio cuando lo haya. **HECHO 2026-09-24, aceptado visualmente por el dueño** («se ve bastante bien»): `docs/referencias/package_raw_v1.glb` (TRELLIS.2, `--no-upsample`, remallado 256) → `tools/blender_cleanup.py --outer-shell` → `assets/models/package_clean_v1.glb`, 800 tris, `glb_check --pivot base` 8/8 entonces y 7/8 tras corregir el punto 4 → instanciado en `src/cargo/package.tscn` a Y = −0,2 m sobre la caja de D81 sin tocar. Procedencia en `CREDITS.md`. **Abiertos, del dueño:** el visual mide 0,400 × 0,319 × 0,362 m dentro de una caja de 0,4 m (8,1 cm de aire arriba, 1,9 cm por delante y por detrás), y lleva tres texturas donde §10.1 punto 4 pide una.
