...
## Handling measurement rig — PERMANENT (r1.3 of M2-T2.2; it was deleted
## once as "temporary" and that left the approved feel without its regression
## ruler). Spawns the bus on flat ground and prints MEASURE lines: rest
## height, 0-60 km/h time, top speed, turn radius, braking from 50 km/h.
## Run: godot --headless --path . -s res://src/tooling/probe_handling.gd
## (always under an external timeout; quit() on every path).
