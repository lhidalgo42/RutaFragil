...
- **r8 (2026-09-24):** corredera de altura completa, ventanas de cabina y mirada 360° del copiloto implementadas; gate humano pendiente. La mirada del copiloto no se replica (no es necesaria para jugar; revisar en M4 si se quiere mostrar la cabeza del copiloto remoto).
...
- **Texturas del paquete embebidas en el GLB, no extraídas** (2026-09-24): `assets/models/package_clean_v1.glb.import` pasa a `gltf/embedded_image_handling=3`. Con el valor por defecto (1) cada importación extrae tres PNG junto al modelo, y versionarlos duplica 1,7 MB en LFS, que en GitHub es permanente. Verificado en un worktree limpio: sin los PNG y con `=3`, `--import` no los recrea y el material toma albedo y normal desde `package_clean_v1.glb::ImageTexture_*`. El dueño confirmó el borrado (R12) y ya no están: `assets/models/` conserva solo `package_clean_v1.glb` y su `.import`. Reimportado sin recrearlos y tests del paquete 14/14 tras la limpieza.
