...
- **Ronda 3 (trazado real, D65) deja pendiente:** (a) las cuadras sin edificios mapeados en OSM quedan vacías: rellenar el frente sin datos con `HouseRow` generadas; (b) relieve del terreno fuera del corredor (hoy plano); (c) soleras con canto redondeado; (d) el pasaje real mide ~8 m y está a 14; las islas de surtidores a 11 m (real 9,5); (e) bandejón, veredas, autos estacionados, paraderos, semáforos, árboles y señales son solo malla mientras el bus sea la caja de D48; (f) la estación y el sitio de combustible están en sitios plausibles, no en la bencinera real (700 m fuera del corredor); (g) `building:levels` viene vacío en 174 de 176 edificios: la altura se estima por tipo y superficie; (h) refresco de datos: correr `python3 tools/osm_to_b0.py` tras reemplazar `tools/osm_departamental_compacto.json` (consulta Overpass documentada en `docs/referencias/B0_ciudad_referencias.html` §4).
- **Ronda 4 (realismo, D67) deja pendiente:** soleras con canto redondeado; relieve del terreno cerca del corredor (hoy solo cerros lejanos); la estación y el sitio de combustible siguen con las cajas lisas de v2 (sin ventanas ni marquesina detallada); los autos estacionados son cajas sin ruedas; los cables no cruzan la avenida; las cebras van solo en la avenida, no en las calles laterales; el shader de fachadas pone ventanas también en muros medianeros. Todo esto es pase de arte (T8.1) salvo lo que Leo pida antes.
- **Ronda 5 (Rancagua, D68) deja pendiente:** (a) el bus con ruedas de T1.1 debe consumir `GravelZone` (`grip_multiplier` 0,55) y la calamina; definir el parámetro de agarre en `TuningTable` (R2) en vez de la constante del zona; (b) el damero real tiene calles de 10–12 m: se usó la avenida ancha de San Martín porque la caja de D48 no cabe en ellas; (c) las soleras de la calle de población son solo malla; (d) la ruta cruza la Alameda sin representarla (bandejón real perpendicular); (e) el paso bajo nivel del tutorial no existe en esta ruta (sí en Departamental); (f) rellenar con `HouseRow`/`fill` los frentes es ~93 % del total: el norte de Rancagua está poco mapeado en OSM; (g) el shader pone grietas también en muros medianeros; (h) refresco de datos: bajar los dos `.osm` por la API 0.6 con las cajas de `docs/referencias/B0_ciudad_referencias.html` §5.2 y correr `python3 tools/osm_to_rancagua.py`.
- **Dirección de arte «ciudad gastada» (D68):** decisión del colaborador con nivel «menos ruina»; Leo confirma o ajusta al revisar la rama.
- **Trampa de pruebas headless:** `MultiMesh.get_instance_transform()` devuelve identidad con el renderer dummy; cualquier prueba sobre MultiMesh debe leer las posiciones que emitió el constructor (`MeshBatcher.positions`, `GrassStrip.positions`), nunca el buffer.
- **Cámara M y foto del mapa (D66)** viven en la ruta greybox; decidir en M8 si el juego tiene minimapa o mapa de pausa propio.
...
## Pendientes de M5-T5.2a ronda 6 (Requínoa)

- **El placeholder no sostiene las curvas del pueblo (para T1.1).** Medido: con `lateral_grip` 8 la caja de D48 derrapa 16 m en una curva de 34 m de radio a 80 km/h y se clava contra las fachadas, o frena a 1,8 km/h y gira en el sitio hasta que salta `stuck`. La vuelta de la ronda 6 se cierra con el nodo `DemoSpeedCap` de la escena de demo a 50 km/h. El bus real necesita agarre lateral propio, caja de cambios o límite por bioma; mientras no exista, cualquier ruta de pueblo debe demostrarse con ese tope.
- **`stuck_seconds` 8 s es corto para un pueblo.** En las esquinas cerradas el bus sale a menos de 2 km/h y el detector salta antes de que enderece; la escena de Requínoa lo sube a 26 s. Revisar el valor por defecto de `DemoDriver` cuando entre el bus real.
- **La rotonda es un añadido de diseño.** Requínoa no tiene ninguna; se insertó en Las Dalias × Caupolicán, a 183 m de la Plaza de Armas, porque el dueño la pidió. Si el arte final quiere fidelidad, hay que quitarla o moverla a un cruce plausible.
