...
## Restraint anchors — the INTEGRATION suite (D69/D84, M2-T2.3): strapped
## packages on the real Playground. Drift vs the anchor is zero by
## construction (ADR-006) and measured over a full demo lap; the crew bumps
## into a strapped box like into a shelf and the corridor stays clear between
## two of them. The mechanics half (occupation, preconditions, parentage,
## collision) lives in restraint_anchor_test.gd (r2.2 split of M2-GATE).
