...
- Input en tests headless (medido en M2-T2.2 r2, 2026-09-13): headless no entrega `InputEvent` de teclado, pero `Input.action_press`/`action_release` sí conducen el estado de acciones; `is_action_just_pressed` se pierde antes del tick de física (flush por frame de proceso) — para flancos, detectar a mano con `is_action_pressed` (ver `src/crew/crew_input.gd` y `tests/crew/crew_input_test.gd`).
